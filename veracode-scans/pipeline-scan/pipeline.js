...
async function pipelineScan(
  ...
  try {
    ...
    const invalidPolicy = await veracodePolicyVerification(
      apiId,
      apiKey,
      policyName,
      breakBuildOnInvalidPolicy
      ...
      for (const resultFileName of failedScan.results) {
        try {
          ...
          await uploadArtifact(
            veracodeArtifactsDir,
            artifactName,
            simplifiedFileName(resultFileName),
            JSON.stringify(resultsJSON, null, 2)
          );
          const isFilterMitigatedFlaws = filterMitigatedFlaws === 'true'
            ? true
            : filterMitigatedFlaws === 'false'
            ? false
            : filterMitigatedFlaws;
          let mitigatedResult = resultsJSON;
          if (isFilterMitigatedFlaws) {
            mitigatedResult = await mitigatedFlaws(
              apiId,
              apiKey,
              appProfileName,
              resultsJSON,
